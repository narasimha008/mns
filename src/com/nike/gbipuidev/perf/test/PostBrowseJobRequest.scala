...
class PostBrowseJobRequest extends Simulation {
  ...
  val MicroserviceScenario = scenario("Browse Job Request")
    ...
    .pause(1)
    .exec(inputfeed,GBIPUIDevServices.postBrowsingJobsRequest)    

  before {
    println("Starting Browse Job Request Load injector test")
  }
  ...
  after {
    println("Completed Browse Job Request Load injector test")
  }
}
