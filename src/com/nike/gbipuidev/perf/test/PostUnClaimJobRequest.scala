...
class PostUnClaimJobRequest extends Simulation {
  ...
  val MicroserviceScenario = scenario("un Claim Job Request")
    .exec(
        addCookie(
    ...
    .pause(1)
    .exec(inputfeed,GBIPUIDevServices.postJobsUnClaimRequest)    

  before {
    println("Starting Un Claim Job Request Load injector test")
  }
  ...
  after {
    println("Completed Un Claim Job Request Load injector test")
  }
}
